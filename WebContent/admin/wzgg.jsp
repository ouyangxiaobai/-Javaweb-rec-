...
<body>
<form  action="wzgg.jsp"  name="f1" method="post">
          ...
          <tr>
            <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
              <tr>
                ...
                <td width="97%" valign="bottom"><span class="STYLE1"> 职场资讯</span></td>
              </tr>
            </table></td>
            <td> 
...
<input type=submit class='' value='查询信息' />
 &nbsp;&nbsp;
<input type=button   class='' value='添加信息' onclick="window.location.replace('ggzdatadictj.jsp')" /> 
&nbsp;&nbsp;</td>
        </tr>
...
if(request.getParameter("datatypeid1")!=null){ 
 new CommDAO().commOper("update zdatadic set datatype ='' where id="+request.getParameter("datatypeid1"));  
} 
if(request.getParameter("datatypeid2")!=null){ 
 new CommDAO().commOper("update zdatadic set datatype ='' where id="+request.getParameter("datatypeid2"));  
} 
String sql = "select * from zdatadic where 1=1 " ;
 if(!pptitle.equals("")){ 
 ...
 sql+= " and ptitle <'"+Info.getDay(endptitle,1)+"' " ;
  ...
  sql +="  and (1!=1 " ; 
  sql +="  or  datatype like'%职场资讯%' " ;
  sql +="  ) ";
  sql +=" order by id desc ";
String url = "wzgg.jsp?1=1&ptitle="+pptitle+""; 
ArrayList<HashMap> list = PageManager.getPages(url,5, sql, request ); 
for(HashMap map:list){ %>
<tr>

        <td height="30" align="center" bgcolor="#FFFFFF" class="STYLE19"><%=map.get("ptitle")%></td>
...
<a href="ggzdatadicxg.jsp?id=<%=map.get("id")%>">修改</a>
&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;

<a onclick="return confirm('确定要删除这条记录吗?')" href="wzgg.jsp?scid=<%=map.get("id")%>">删除</a>
</td>
</tr>
<%}%>
...
<script language=javascript> 
function update(no){ 
pop('ggzdatadicxg.jsp?id='+no,'信息修改',550,'100%') 
}
</script> 
<script language=javascript> 
function add(){ 
pop('ggzdatadictj.jsp','信息添加',550,'100%') 
}
</script> 
<%@page import="util.Info"%> 
