...
<body>
<form  action="studentcx.jsp"  name="f1" method="post">
          ...
          <tr>
            <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
              <tr>
                ...
                <td width="97%" valign="bottom"><span class="STYLE1"> 我的求职信息查看</span></td>
              </tr>
            </table></td>
            <td> 
    ...
    <td><table width="100%" border="0" cellpadding="0" cellspacing="1" class="mytab"   onmouseover="changeto()"  onmouseout="changeback()">
      ...
      <tr>
        <td colspan=7 height="30"   align="left" bgcolor="#FFFFFF" class="STYLE19">
          <label></label>&nbsp;&nbsp;&nbsp;
<% 
HashMap smap = (HashMap)session.getAttribute("admin");
String username = smap.get("uname").toString();
...
new CommDAO().delete(request,"emp"); 
      ...
      String msuname = request.getParameter("msuname");
     
      if(msuname!=null&&!msuname.equals("")){
    	  new CommDAO().commOper("update emp set status='面试' where uname='"+msuname+"'");
...
String url = "studentcx.jsp?1=1&uname="+puname+"&tname="+ptname+"&savetime="+psavetime+""; 
...
<%if(map.get("status").toString().equals("待处理")&&smap.get("utype").toString().equals("企业")){ %>
&nbsp;|&nbsp;
 <a href="studentcx.jsp?msuname=<%=map.get("uname")%>">通知面试</a>
 <%} %>
</td>
</tr>
<%}%>
