...
String tname = Info.getGB2312Str(request.getParameter("tname")) ;

String tsql = "select * from zxjl where pid='"+id+"' or id='"+id+"' " ;
   
  tsql +="  and (1!=1 " ; 
  tsql +="  or  uname like'%"+tname+"%'  or  touname like'%"+tname+"%' " ;
  tsql +="  ) ";
  tsql +=" order by id  ";
String url = "szxjlcx.jsp?1=1&id="+id ; 
ArrayList<HashMap> list = PageManager.getPages(url,55, tsql, request ); 
for(HashMap map:list){  
 str+="  &nbsp;&nbsp; <font color=orange>";
 str+= map.get("uname")+" &nbsp;  "+map.get("savetime")+" : <br />  </font>&nbsp;&nbsp; ";
 str+= map.get("tcontent")+"<br />"; 
}
...
String id = request.getParameter("id");
String tname = Info.getGB2312Str(request.getParameter("tname"));
String tuname = Info.getGB2312Str(request.getParameter("touname")) ;
String tcontent = Info.getGB2312Str(request.getParameter("tcontent")) ;
String tsql = "insert into zxjl values(null,'"+tname+"','"+tuname+"','"+tcontent+"','"+Info.getDateStr()+"','"+id+"') " ;
