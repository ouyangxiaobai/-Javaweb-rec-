...
A:active {
	TEXT-DECORATION: none
       ...
       alert("请先登录");
       window.location.replace("/jsp_qzzp_sys/index.jsp");
       </script>
       <%} %>
          ...
          <TD vAlign=top>
            <TABLE border=0 cellSpacing=0 cellPadding=0 width="100%">
              ...
              <TR>
                ...
                if(content!=null)
                {
                dao.commOper("insert into messages (saver,savetime,content,resaver,recontent,resavetime) values('"+umap.get("uname")+"','"+Info.getDateStr()+"','"+content+"','','','')");
                }
                 }
                 %>
                ...
                <form action="/jsp_qzzp_sys/messages.jsp?f=f"   method="post" name="f1"  >
                <TABLE 
...
<%
                              if(!m.get("resaver").equals("")){
                            	  HashMap aumap = null;
                            	  try{
                            		  aumap = dao.select("select * from sysuser where uname='"+m.get("resaver")+"'").get(0);
                            	  }catch(Exception e){
                            		  e.printStackTrace();
                            	  }
